...
-- 4) id'si 5 olan personelin id,isim, yaş ve email bilgilerini listeleyiniz
...
select * from personel where maas<>21000 ; --SELECT * FROM personel WHERE maas is not 21000
...
--ilike:buyuk kucuk harf duyarliligi icin kullaniliyor.
...
select * from personel order by yas ; --siralama diyince order by aklimiza gelmeli
--order by'in dogal siralamasikucukten buyugedir(asc'de eklenebilir)
...
select *from personel order by yas desc ; --desc buyukten kucuge siralamayi saglar
...
select * from personel order by maas desc limit 3 ; --limit sinirlandirma getirir.
