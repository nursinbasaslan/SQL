...
select cinsiyet, avg(maas) 
FROM personel
group by cinsiyet ;
-- Maas ortalamasını bayanlar ve baylar olarak sorgulayınız(tam sayi cinsinden sonuc-->)
...
select ulke, cinsiyet, count(*)    --count(*) tablodaki her veriyi saydirir
as calisan_sayisi, round( avg(yas)) as yas_ortalamasi from personel
group by ulke, cinsiyet ; --tam sayi olarak bulmak icin 'round'
